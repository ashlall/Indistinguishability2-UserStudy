...
vector<point_t*> get_extreme_pts(vector<point_t*>& ext_vec)
{
	...
	// it is possible for there to be no feasible points as we don't eliminate unpreferred points when they are not (1+epsilon)-dominated
	// resulting in that same pair being shown again and being infeasible.
	if (feasible_pt == NULL)
	{
	...
	//cout << "computed feasible point: ";
	//print_point(feasible_pt);
	...
	// TODO: Release these points in the future to prevent memory leakage. Need to keep them around for MINC.
	//for(int i = 0; i < ext_vec.size(); i++)
	//{
...
// hyperplane pruning
// check if p_i (1+epsilon)-dominates p_j
int hyperplane_dom(point_t* p_i, point_t* p_j, vector<point_t*> ext_pts, double epsilon)
{
	int dim = p_i->dim;

	point_t* scaled_pj = scale(1 + epsilon, p_j);
	point_t* normal = sub(p_i, scaled_pj);   
...
int conical_hull_dom(point_t* p_i, point_t* p_j, hyperplane_t* hp, vector<point_t*> hyperplanes, vector<point_t*> ext_vec, double epsilon)
{
	...
	point_t* scaled_pj = scale(1 + epsilon, p_j);
	point_t* minus = sub(scaled_pj, p_i);
...
int dom(point_t* p_i, point_t* p_j, vector<point_t*> ext_pts, hyperplane_t* hp, vector<point_t*> hyperplanes, vector<point_t*> ext_vec, int dom_option, double epsilon)
{
	if(dom_option == HYPER_PLANE) // hyperplane pruning
	  return hyperplane_dom(p_i, p_j, ext_pts, epsilon);
	else // conical hull pruning
	  return conical_hull_dom(p_i, p_j, hp, hyperplanes, ext_vec, epsilon);
}
...
void sql_pruning(point_set_t* P, vector<int>& C_idx, vector<point_t*>& ext_vec, double& rr, int stop_option, int dom_option, double epsilon)
{
	...
	// if these conditions are infeasible, just return
	if (ext_pts.size() == 0) return;
	...
	for (int i = 0; i < C_idx.size(); ++i)
	{
		...
		for (int j = 0; j < index && !dominated; ++j)
		{
		  ...
		  if(dom(P->points[ sl[j] ], pt, ext_pts, hp, hyperplanes, ext_vec, dom_option, epsilon))
				dominated = 1;
		}
		...
		if (!dominated)
		{
			...
			for (int j = 0; j < m; ++j)
			{
			  ...
			  if(!dom(pt, P->points[sl[j]], ext_pts, hp, hyperplanes, ext_vec, dom_option, epsilon))
					sl[index++] = sl[j];
			}
...
void rtree_pruning(point_set_t* P, vector<int>& C_idx, vector<point_t*>& ext_vec, double& rr,  int stop_option, int dom_option, double epsilon)
{
	...
	while (!heap.empty())
	{
		...
		if (n->attribute != LEAF)
		{
		  ...
		  for (int j = 0; j < index && !dominated; ++j)
		    if(dom(P->points[ sl[j] ], TRpt, ext_pts, hp, hyperplanes, ext_vec, dom_option, epsilon))
		      dominated = 1;
		...
		else
		{
		  ...
		  int dominated = 0;
		  for (int j = 0; j < index && !dominated; ++j)
		    if(dom(P->points[ sl[j] ], P->points[ C_idx[idx] ], ext_pts, hp, hyperplanes, ext_vec, dom_option, epsilon))
		      dominated = 1;
		  ...
		  for (int j = 0; j < m; ++j)
		  {
		    if(!dom(P->points[C_idx[idx]], P->points[sl[j]], ext_pts, hp, hyperplanes, ext_vec, dom_option, epsilon))
		      sl[index++] = sl[j];
		  }
