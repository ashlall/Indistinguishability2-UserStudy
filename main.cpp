...
// TT is not added as we are only doing s = 2
void run_vary_s(point_set_t* P, point_set_t* cskyline, double epsilon, double delta, int s, int q, bool real, bool fake)
{
  int dim = P->points[0]->dim;
  int Qcount, Csize;
  ...
  for(q = MIN_Q; q <= MAX_Q; q += 5)
      ...
      for(int repeat = 0; repeat < REPEATS; ++repeat)
	{
	  ...
	  if (fake)
        ...
        alpha = max_utility_TT(cskyline, u, s, epsilon, delta, q, Qcount, Csize);
	      avg_alpha[TT][q] += alpha/REPEATS;
...
// TT is not included as we are not doing user error
void run_vary_delta(point_set_t* P, point_set_t* cskyline, double epsilon, double delta, int s, int q, bool real, bool fake)
{
  int dim = P->points[0]->dim;
  int Qcount, Csize;
...
void run_false_positives(point_set_t* P, point_set_t* cskyline, double epsilon, double delta, int s, int q, bool real, bool fake)
{
    ...
    if (fake)
      {
        ...
        alpha = max_utility_TT(cskyline, u, s, epsilon, delta, q, Qcount, Csize);
	      C_size[TT] += ((double)Csize)/REPEATS;
      }
  ...
  FILE* fp = fopen(filename, "w");
  printf("%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", C_size[SQUEEZEU], C_size[TT], C_size[UHRAND], C_size[MIN_D], C_size[MIN_R], C_size[Breakpoint], C_size[REAL]);
...
void run_time(point_set_t* P, point_set_t* cskyline, double epsilon, double delta, int s, int q, bool real, bool fake)
{
  ...
  FILE* fp = fopen(filename, "w");
  printf("%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", t[SQUEEZEU], t[TT], t[UHRAND], t[MIN_D], t[MIN_R], t[Breakpoint]);
...
void run_vary_T(point_set_t* P, point_set_t* cskyline, double epsilon, double delta, int s, int q)
{
  ...
  for(int T = MIN_T; T <= MAX_T; T += 20)
    {
      printf("%d\t%lf\t%lf\t%lf\n", T, avg_alpha[MIN_D][T], avg_alpha[MIN_R][T], avg_alpha[Breakpoint][T]);
...
void run_vary_n(double epsilon, double delta, int s, int q, bool real, bool fake)
{
  ...
  for(int i = 0; i < 4; ++i)
    {
      printf("%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", n, avg_alpha[SQUEEZEU][i], avg_alpha[TT][i], avg_alpha[UHRAND][i], avg_alpha[MIN_D][i], avg_alpha[MIN_R][i], avg_alpha[Breakpoint][i], t[SQUEEZEU][i], t[TT][i], t[UHRAND][i], t[MIN_D][i], t[MIN_R][i], t[Breakpoint][i]);
...
void run_vary_d(double epsilon, double delta, int s, int q, bool real, bool fake)
{
  ...
  for(int i = 0; i < 5; ++i)
    {
      printf("%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", dim, avg_alpha[SQUEEZEU][i], avg_alpha[TT][i], avg_alpha[UHRAND][i], avg_alpha[MIN_D][i], avg_alpha[MIN_R][i], avg_alpha[Breakpoint][i], t[SQUEEZEU][i], t[TT][i], t[UHRAND][i], t[MIN_D][i], t[MIN_R][i], t[Breakpoint][i]);
      fprintf(fp, "%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", dim, avg_alpha[SQUEEZEU][i], avg_alpha[TT][i], avg_alpha[UHRAND][i], avg_alpha[MIN_D][i], avg_alpha[MIN_R][i], avg_alpha[Breakpoint][i], t[SQUEEZEU][i], t[TT][i], t[UHRAND][i], t[MIN_D][i], t[MIN_R][i], t[Breakpoint][i]);
      dim += 1;
    }
  fclose(fp);
